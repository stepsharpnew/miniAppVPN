...
- **Bandwidth Management System**: Multi-tier bandwidth limiting with three configurable tiers (Free, VIP, Super VIP)
  - Web UI for managing bandwidth tier settings
  - Automatic traffic control (tc) rules application  
  - Tier selection when creating servers (applies to all clients)
  - Ability to change server tier (updates all clients)
  - Real-time bandwidth limit updates for running servers
  - Recommended limits: Free (6 Mbit/s), VIP (50 Mbit/s), Super VIP (Unlimited)
- **Improved IP Address Management**: Fixed IP allocation bugs
  - IP addresses from deleted clients are now reused
  - Last octet overflow prevention (no more 10.0.0.257)
  - More efficient IP allocation algorithm
- **Tab Navigation**: Added tabbed interface for Servers and Bandwidth Settings
- **Enhanced Client Display**: Client list now shows assigned bandwidth tier badges
- **New API Endpoints**:
...
- Fixed line endings issue in shell scripts causing deployment failures on Linux
- Fixed IP address allocation bugs (overflow and duplicate IPs)
- Fixed bandwidth limit application at server startup

### Changes

- Server creation now includes bandwidth tier selection
- All clients on a server automatically inherit the server's bandwidth tier
- Server startup now automatically applies bandwidth limits to all clients
- Added "Change Tier" button for servers in web UI
- Added docker-compose.yml for easier deployment
- Added .gitattributes to prevent line ending issues
- Updated documentation with bandwidth management instructions
