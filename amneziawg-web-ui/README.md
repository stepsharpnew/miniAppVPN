...
*   **Bandwidth Management**: Multi-tier bandwidth limiting system (Free, VIP, Super VIP)
...
Returns all configured bandwidth tiers (Free, VIP, Super VIP) with their settings.

#### Update Bandwidth Tier

```yaml
PUT /api/bandwidth/tiers/{tier}
Content-Type: application/json
...
Updates settings for a specific tier. Set `limit_mbit` to 0 for unlimited bandwidth.

#### Update Server Bandwidth Tier

```yaml
PUT /api/servers/{server_id}/tier
Content-Type: application/json
...
Changes the bandwidth tier for the server and all its clients. Available tiers: `free`, `vip`, `super_vip`.

**Bandwidth Tier Guidelines:**
...
### Docker Run Example (Recommended)
...
**Important:** Make sure to include `--device /dev/net/tun:/dev/net/tun` for VPN functionality!
